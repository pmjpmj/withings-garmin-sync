...
- [09 - Withings developer portal credentials](issues/09-withings-developer-credentials.md): operator gets `client_id`+`client_secret` at **https://developer.withings.com/dashboard/** (create app there; personal account fine, org account only *recommended*); **EU Public Cloud only** (free, no contract → `wbsapi.withings.net`; US Medical Cloud needs a contract); redirect URI must be registered as `http://localhost:8765/`; secrets viewable/rotatable in the dashboard; free-tier limits comfortably above this tool's one-user polling.
- [10 - Withings OAuth authorize URL missing required `state`](issues/10-withings-oauth-state.md): Withings requires `state` on `authorize2` (`required: true`). CLI now generates a random per-run state, sends it, and validates the echoed value when the operator pastes a full redirect URL (bare codes still accepted; mismatch = exit 4). Token exchange takes no `state`.
- [11 - Garmin writes returned HTTP 400](issues/11-garmin-write-400.md): root cause was a **duplicate `Content-Type` header** on the wire (pushed explicitly + added by `.json()`); the edge rejected it with an empty-body 400 for every payload/TLS/protocol variant. Fixed; raw-wire + exact-header-set tests pin the request. Garmin write endpoints **require** millisecond timestamps (no-fraction → 500) and accept 3-dp kg values and backdated dates. HTTP/1.1 + `native-tls` is the working stack (hyper's h2 fingerprint was edge-rejected).
- [14 - Per-metric sync subcommands](issues/14-per-metric-sync-subcommands.md): `sync` gained `weight`/`bp`/`all` subcommands (bare `sync` = `all`, ADR-0005). Withings reads are scoped per metric (`sync weight` requests meastype `1`, `sync bp` requests `9,10,11`); a single-metric run never fetches or writes the other metric; reports are metric-aware; exit codes unchanged. Flags go on the subcommand (`sync weight --apply`); flags before the subcommand exit 2. Cadence stays external (cron: weight `0 */2 * * *`, BP `15 21 * * *`).
- [15 - Per-service auth subcommands](issues/15-per-service-auth-subcommands.md): `auth` gains `withings`/`garmin` subcommands (bare `auth` = both, back-compatible); each command replaces only its own section of `tokens.json` (read-modify-write); `auth garmin` needs no config; per-service commands are always fully interactive; sync-side auth errors name the service to re-auth. Decision recorded (ADR-0006); implemented (2026-09-05, `tests/auth_services.rs`).
